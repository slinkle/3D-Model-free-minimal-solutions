...
savepathw = strcat(datapath,'AllNworlds_',num2str(totalNum),'_outlier_big3dnoise.mat');

sample_pnum=totalNum;
AllNworlds={};
phi_min_max=[0,60;120,180;240,300]./180*pi;
...
%     featureNoise=featureNoises(Id);
    ...
    %         while true
            theta = (0 + (pi/2-(0)).*rand(1,1));
            phi=(phi_min_max(wid,1) + (phi_min_max(wid,2)-(phi_min_max(wid,1))).*rand(1,1));
        ...
        current_N_worlds{wid}=worlds;
    end
    AllNworlds = [AllNworlds;current_N_worlds];

end% end for each dismatch level


save(savepathw,'AllNworlds');
